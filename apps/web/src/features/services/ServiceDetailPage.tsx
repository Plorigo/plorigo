...
// the connection string from the service's env vars — POSTGRES_USER/PASSWORD/DB, written at
// provision time — plus the service's internal host and container port; siblings reach it at
...
function ConnectionPanel({ service }: { service: Service }) {
  const envVars = useEnvVars(service.id);
  const rows = envVars.data ?? [];
  const get = (key: string) => rows.find((v) => v.key === key)?.value ?? "";
