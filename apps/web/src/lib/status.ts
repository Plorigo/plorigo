...
export function statusIntent(status: string): Intent {
  ...
  if (["building", "queued", "protected", "pending", "info", "assigned", "pulling", "starting", "healthcheck", "routing", "connecting", "setting up", "provisioning"].includes(status)) {
    return "info";
  }
  if (["degraded", "needs restore test", "warning", "warn"].includes(status)) {
    return "warning";
  }
  if (["failed", "offline", "unavailable", "blocked", "attention", "error", "danger", "setup failed"].includes(status)) {
    return "danger";
  }
  // "unknown" (offline/never-connected readiness) and anything unrecognized fall through to
  // neutral — grey, not an alarm.
  return "neutral";
}
