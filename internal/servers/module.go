...
// Deps are what the servers module needs. Audit and Policy are CONSUMER-DEFINED ports
// (authz.Authorizer is satisfied by *policy.Service, Recorder by *audit.Service), wired in
// internal/app — servers imports neither module.
type Deps struct {
	DB     *database.DB
	Audit  Recorder
...
// Module is the servers module: the only wiring surface other code touches.
type Module struct {
	service Service
}
...
// Route returns the ServerService mount path and handler. opts carries the app-wide
// interceptors (e.g. the auth interceptor).
func (m *Module) Route(opts ...connect.HandlerOption) (string, http.Handler) {
	return controlplanev1connect.NewServerServiceHandler(&handler{svc: m.service}, opts...)
}
